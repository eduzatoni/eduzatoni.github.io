! function(e) {
    ...
    Object.defineProperty(t, "__esModule", {
        value: !0
    }), t.patchConsoleLogging = t.isValidPayload = t.payloadMessageDataProperties = t.checkDevice = t.isDeviceRegistered = t.urlB64ToString = t.urlB64ToUint8Array = t.getGlobal = t.getVersion = void 0;
    const a = o(i(2));
    ...
    t.getVersion = function() {
        return "2.2.0"
    }, t.getGlobal = r, t.urlB64ToUint8Array = function(e) {
        const t = c(e),
        ...
        return t && (void 0 !== i && i)
    }, t.checkDevice = l, t.payloadMessageDataProperties = {
        id: "string",
        sid: "string",
    ...
    }, t.patchConsoleLogging = function(e) {
        const t = e || function() {},
            i = ["trace", "debug", "info", "warn", "error", "log"],
            n = r(),
            s = n.console = n.console || {};
        for (const e of i) s[e] = t
    }
}, function(e, t, i) {
    "use strict";
    Object.defineProperty(t, "__esModule", {
    ...
    t.decode = function(e, t) {
        ...
        try {
            return JSON.parse(n.urlB64ToString(e.split(".")[i]))
        } catch (e) {
            return console.log("Error decoding token", e), null
        }
    }
}, function(e, t, i) {
    "use strict";
    ...
    class s {
        ...
        async postEvents(e) {
            ...
            if (!t) return console.log("Error: No client ID set for the browser!"), {
                success: !1
            };
            const i = await this.storage.getContactToken();
            if (!i) return console.log("Error: No contact token set!"), {
                success: !1
            };
            const s = await this.storage.getClientState();
            if (!s) return console.log("Error: No client state set!"), {
                success: !1
            };
            const o = await this.apiEndpoint("client/events"),
                a = await this.desRequest.post(o, e, {
                    clientId: t,
                    clientState: s,
                    contactToken: i
                });
            if (n.includes(a.status)) return {
                success: !0,
                statusCode: a.status
            }; {
                const e = await a.json();
                return console.log("Error posting events to device event service", a.status, JSON.stringify(e)), {
                    success: !1,
                    statusCode: a.status
                }
            }
    ...
    class n {
        ...
        async storeClientDetails() {
            ...
            if (!e) return console.log("Error: No client ID set for the browser!"), !1;
            ...
            if (204 === s.status) return await this.saveClientState(s), !0; {
                ...
                return console.log("Error storing client info", s.status, JSON.stringify(e)), !1
            }
        }
        async linkClientToContact(e) {
            const t = await this.storage.getClientId();
            if (!t) return console.log("No client ID set for the browser"), !1;
            const i = await this.storage.getClientState();
            if (!i) return console.log(`No client state for client "${t}"!`), !1;
            const {
                anonymous: n,
                body: s
            } = this.toContactRequestBodyData(e), o = `${await this.apiEndpoint("client/contact")}${n?"?anonymous=true":""}`, a = await this.meClientServiceRequest.post(o, s, {
                clientId: t,
                clientState: i
            });
            if (200 === a.status) {
                await this.saveClientState(a);
                const e = await a.json();
                return e.contactToken && e.refreshToken ? (await Promise.all([this.storage.setContactToken(e.contactToken), this.storage.setRefreshToken(e.refreshToken)]), !0) : (console.log("At least one of the expected response parts missing!"), !1)
            } {
                const e = await a.json();
                return console.log("Error linking contact to client", a.status, JSON.stringify(e)), !1
            }
        }
        async generateAccessToken() {
            const e = await this.storage.getClientId();
            if (!e) return console.log("No client ID set for the browser"), !1;
            const t = await this.storage.getRefreshToken();
            if (!t) return console.log("Unable to refresh contact token as refreshtoken is missing!"), !1;
            const i = await this.storage.getClientState(),
                n = await this.apiEndpoint("client/contact-token"),
                s = {
                    refreshToken: t
                },
                o = await this.meClientServiceRequest.post(n, s, {
                    clientId: e,
                    clientState: i
                });
            if (200 === o.status) {
                ...
                return e.contactToken ? (await this.storage.setContactToken(e.contactToken), !0) : (console.log("contactToken is not part of response body!"), !1)
            } {
                const e = await o.json();
                return console.log("Error refreshing the contact token", o.status, JSON.stringify(e)), !1
            }
        }
        async registerPushToken(e) {
            ...
            if (!n) return console.log("Unable to register subscription as contactToken is missing!"), !1;
            const s = await this.apiEndpoint("client/push-token"),
                o = {
                    pushToken: e
                },
                a = await this.meClientServiceRequest.put(s, o, {
                    clientId: t,
                    clientState: i,
                    contactToken: n
                });
            if (204 === a.status) return await this.saveClientState(a), !0; {
                const e = await a.json();
                return console.log("Error registering the subscription", a.status, JSON.stringify(e)), !1
            }
        }
        async removePushToken() {
            ...
            if (!t) return console.log(`No client state for client "${e}"!`), !1;
            const i = await this.storage.getContactToken();
            if (!i) return console.log("Unable to register subscription as contactToken is missing!"), !1;
            ...
            if (204 === s.status) return await this.saveClientState(s), !0; {
                const e = await s.json();
                return console.log("Error removing a subscription", s.status, JSON.stringify(e)), !0
            }
        }
        async apiEndpoint(e) {
            const t = await this.storage.getAppCode();
        ...
        async saveClientState(e) {
            ...
            t ? await this.storage.setClientState(t) : console.log("Error: X-Client-State not found in response header!")
        }
    ...
    class o {
        ...
        setDBValue(e, t) {
            return this.openIndexDB().then((i => new Promise(((n, s) => {
                const o = i.transaction([this.name], "readwrite").objectStore(this.name).put({
                    key: e,
                    value: t
                });
                o.onsuccess = () => n(e), o.onerror = e => s(e)
            }))))
        }
        getDBValue(e) {
            return this.openIndexDB().then((t => new Promise(((i, n) => {
                const o = t.transaction(this.name).objectStore(this.name).get(e);
                o.onsuccess = () => {
                    const {
                        result: e
                    } = o;
                    return e ? i(e.value) : n(new Error(s))
                }, o.onerror = e => n(e)
            }))))
        }
    ...
    const n = i(11),
        s = i(6),
        o = i(7),
        a = s.MEWebPushDb.create(o.IndexDb.create()),
        r = n.EmarsysServiceWorker.create(a);
    self.addEventListener("push", (e => {
        r.onPush(e)
    })), self.addEventListener("notificationclick", (e => r.onNotificationClick(e))), self.addEventListener("install", (e => r.onInstall(e)))
}, function(e, t, i) {
    "use strict";
    Object.defineProperty(t, "__esModule", {
        value: !0
    ...
    class c {
        ...
        onInstall(e) {
            return e.waitUntil(this.handleInstall())
        }
        onPush(e) {
            if (!("showNotification" in self.registration)) return void console.log("Show notification is not enabled");
            const t = e.data && e.data.json() ? e.data.json() : {};
            if (!s.isValidPayload(t)) return void console.log("Invalid payload", t);
            const i = t.messageData.notificationSettings;
            return e.waitUntil(Promise.all([this.getNotificationOption(t, n.pushTitleProperty, (() => this.webPushDb.getDefaultNotificationTitle(""))), this.getNotificationOption(i, n.pushIconProperty, (() => this.webPushDb.getDefaultNotificationIcon(void 0)))]).then((([e, i]) => this.showNotification(t.message, t, e, i))))
        }
        onNotificationClick(e) {
            e.notification.close();
            const t = e.notification.data;
            if (t.messageData.notificationSettings && t.messageData.notificationSettings[n.pushLinkProperty]) return e.waitUntil(Promise.all([self.clients.openWindow(t.messageData.notificationSettings[n.pushLinkProperty]), this.reportOpen(t)]))
        }
        showNotification(e, t, i, n) {
            const s = {
                body: e,
                data: t,
                icon: n,
        ...
        async handleInstall() {
            try {
                await this.setupLogging(), console.log("Install handler"), console.log("storing service worker version", "2.2.0"), await this.webPushDb.setServiceWorkerVersion("2.2.0"), console.log("skipping waiting"), await self.skipWaiting(), console.log("install done")
            } catch (e) {
                console.error(e, "install error!")
            }
        }
        async setupLogging() {
            await this.webPushDb.getLoggingEnabled() || s.patchConsoleLogging()
        }
        async reportOpen(e) {
            var t;
            const i = await this.getDeviceEventService();
            if (i) try {
                console.log("Reporting open to DES", e);
                ...
                let a = await i.postEvents(o);
                a.success || 401 !== a.statusCode || (a = await this.retrySendAfterContactTokenRefresh(i, o))
            } catch (e) {
                console.error("Fatal error while reporting open!", e.message, e)
            } else console.error("Cannot report open! DES not initialized!", e)
        }
        async retrySendAfterContactTokenRefresh(e, t) {
            const i = await this.getMeClientService();
            if (!i) return console.error("Unable to get the ME client service!"), {
                success: !1
            };
            return await i.generateAccessToken() ? e.postEvents(t) : (console.error("Refresh of access token failed!"), {
                success: !1
            })
        }
        async getDeviceEventService() {
            try {
                if (!this.meDeviceEventService) {
                    ...
                    this.meDeviceEventService = o.MEDeviceEventService.create(e, r.MEV3ApiRequest.create(), this.webPushDb)
                }
                return this.meDeviceEventService
            } catch (e) {
                console.error("Error initializing device event service!", e.message, e)
            }
        }
        async getMeClientService() {
            try {
                if (!this.meClientService) {
                    ...
                    this.meClientService = a.MEClientService.create(e, r.MEV3ApiRequest.create(), this.webPushDb)
                }
                return this.meClientService
            } catch (e) {
                console.error("Error initializing client service!", e.message, e)
            }
        }
        static create(e) {
            return new c(e)
